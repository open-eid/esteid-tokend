...
public:
    EstEIDKeyHandle(EstEIDToken &token, const Tokend::MetaRecord &metaRecord, EstEIDKeyRecord &record);
    ~EstEIDKeyHandle();
...
private:
    EstEIDToken &mToken;
    void _generateRsaSignature(const CssmData &input, CssmData &signature);
};
...
public:
    ...
    virtual Tokend::KeyHandle *keyHandle(Tokend::TokenContext *tokenContext, const Tokend::MetaRecord &metaRecord, Tokend::Record &record) const;
};
